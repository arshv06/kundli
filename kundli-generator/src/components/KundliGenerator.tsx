...
import YogaAnalysis from "./YogaAnalysis";
...
const aspectRules: Record<string, number[]> = {
  // values are house offsets (difference) NOT house numbers
  Su: [6],      // 7th aspect (opposition)
  Mo: [6],
  Ma: [3, 6, 7],   // 4th, 7th, 8th aspects
  Me: [6],
  Ju: [4, 6, 8],   // 5th, 7th, 9th aspects
  Ve: [6],
  Sa: [2, 6, 9],   // 3rd, 7th, 10th aspects
  Ra: [4, 6, 8],
  Ke: [4, 6, 8],
  Ur: [6],
  Ne: [6],
...
const aspectTypeLabel: Record<number, string> = {
  2: '3rd', 3: '4th', 4: '5th', 6: '7th', 7: '8th', 8: '9th', 9: '10th'
};
...
const KundliFace: React.FC = () => {
  ...
  const [houseStrengths, setHouseStrengths] = useState<Record<number, {strength: string, color: string}>>({});
  const [dataset, setDataset] = useState<any>({});
  const [modal, setModal] = useState<{open: boolean, house: number|null}>({open: false, house: null});
  const [chartType, setChartType] = useState<'regular' | 'd9'>('regular');
  ...
  const handleChartTypeChange = async () => {
    ...
    if (showChart) {
      const res = await fetch("http://localhost:5000/api/kundli", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({...form, chart_type: newType})
      });
      const data = await res.json();
      setSignPlanets(data.sign_planets);
      setAscSign(data.asc_sign);
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/ai-analysis", {
        ...
        body: JSON.stringify({
          question: aiQuestion,
          user_name: form.name || "friend",
          kundli_data: {
            asc_sign: ascSign,
            chart_type: chartType,
            sign_planets: signPlanets,
            house_descriptions: houseDescriptions
          }
        })
      });
      const data = await res.json();
      ...
      <header className="kundli-header">
        ...
        {showChart && (
          <div style={{display:'flex',alignItems:'center',gap:'1rem'}}>
            ...
            >
              Switch to {chartType === 'regular' ? 'D9' : 'D1'}
            </button>
            <LegendBox />
          </div>
        )}
      ...
      <main className="kundli-main-centered" style={{position:'relative',flexDirection:'row',display:'flex',alignItems:'flex-start',justifyContent:'center'}}>
        ...
        {!showChart ? (
          <form className="kundli-form-centered" onSubmit={handleSubmit}>
            ...
            <label>Name<br/><input type="text" name="name" value={form.name} onChange={handleChange} placeholder="Your name" required /></label>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:20,width:'100%',maxWidth:1200}}>
            {/* Top row: Chart + Aspect Filter + Aspect Table */}
            <div style={{display:'flex',gap:20,alignItems:'flex-start'}}>
                             {/* Left column: Aspect Filter + Chart side by side */}
               <div style={{display:'flex',gap:20,flex:1.3}}>
                 {/* Aspect Filter Box */}
                 <div className="kundli-aspect-sidebar" style={{width:220,flexShrink:0}}>
                     ...
                     {allPlanets.map(p => (
                       <label key={p} style={{display:'flex',alignItems:'center',marginBottom:6,cursor:'pointer',fontWeight:600}}>
                         ...
                         <span style={{color:'#fff',fontWeight:400}}>{planetLabels[p]}</span>
                                  ...
                                  <div className="kundli-svg-outer" style={{flex:1}}>
                   <div style={{display:'flex',justifyContent:'center',alignItems:'center',marginBottom:'0.7rem'}}>
                     <h2 style={{color:'#ffb300',margin:0,fontSize:'1.1rem'}}>{chartType === 'regular' ? 'Birth (D1)' : 'Navamsa (D9)'}</h2>
                   </div>
                  <svg
                    ref={svgRef}
                    viewBox={`0 0 ${S} ${S}`}
                    ...
                    width="98vw"
                    height="98vw"
                    style={{maxWidth:500,height:500,display:'block',margin:'0 auto'}}
                    onMouseMove={e => {
                  ...
                  >
                    ...
                    {houseCenters.map((coord, i) => {
                      const houseNum = i + 1;
                      const houseStrength = houseStrengths[houseNum];
                      const signColor = houseStrength?.color || '#fff';
                      
                      return (
                        <g key={i} style={{cursor:'pointer'}} onClick={()=>setModal({open:true,house:houseNum})}>
                          {/* House number */}
                          <text
                            x={coord.x}
                            y={coord.y - 18}
                          ...
                          {/* Sign name - colored based on house strength */}
                          <text
                            x={coord.x}
                            y={coord.y + 2}
                            textAnchor="middle"
                  ...
                  <HouseModal />
                             ...
                             {/* Right column: Aspect Table */}
               <div className="kundli-aspect-table-outer" style={{flex:0.7,height:'fit-content',maxHeight:'500px',overflowY:'auto'}}>
            ...
            <YogaAnalysis
              signPlanets={signPlanets}
              ascSign={ascSign}
              dataset={dataset}
            />
            
            {/* Detailed House Analysis Section */}
            <div className="kundli-detailed-analysis">
              <div className="analysis-header">
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    background: showDetailedAnalysis ? '#ff6b6b' : '#4CAF50',
                    color: '#fff',
                    border: 'none',
                    borderRadius: '6px',
                    cursor: 'pointer',
                ...
                >
                  {showDetailedAnalysis ? 'Hide Analysis' : 'Show Analysis'}
                </button>
              </div>
              
              {showDetailedAnalysis && (
                <div className="analysis-content">
                  <div className="house-strength-legend">
                    <h4>House Strength Legend:</h4>
                    <div className="legend-items">
                      <div className="legend-item">
                        <div className="legend-color" style={{ backgroundColor: '#90EE90' }}></div>
                        <span>Strong House (Favorable)</span>
                      </div>
                      <div className="legend-item">
                        <div className="legend-color" style={{ backgroundColor: '#FFD700' }}></div>
                        <span>Neutral House (Balanced)</span>
                      </div>
                      <div className="legend-item">
                        <div className="legend-color" style={{ backgroundColor: '#FFB6C1' }}></div>
                        <span>Challenging House (Needs Attention)</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="houses-grid">
                    {Array.from({ length: 12 }, (_, i) => {
                      ...
                      const housePlanets = signPlanets[houseSign] || [];
                      const aspectsToHouse = aspects.filter(a => a.toHouse === houseNum);
            ...
            {/* AI Analysis Box - Moved to the end */}
            <div className="kundli-ai-box">
              <h3>Ask a question!</h3>
              <div className="kundli-ai-input">
                <input
                  type="text"
                ...
                <button
                  ...
                  style={{
                    marginTop: '8px',
                    padding: '10px 20px',
                    background: aiLoading ? '#666' : '#ff6b6b',
                    color: '#fff',
                    border: 'none',
                    borderRadius: '6px',
              ...
              {aiResponse && (
                <div className="kundli-ai-response">
                  <div style={{
                    background: '#2a2a2a',
                    padding: '16px',
                    borderRadius: '8px',
